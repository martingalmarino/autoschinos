...
const Navbar: React.FC = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchTerm.trim()) {
      // Buscar por término general (puede ser marca o modelo)
      navigate(`/catalogo?search=${encodeURIComponent(searchTerm.trim())}`);
    } else {
      // Si está vacío, ir al catálogo completo
      navigate('/catalogo');
    }
  };
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 bg-gradient-to-b from-gray-50 to-white shadow-lg z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Search Bar - Ancho completo hasta Contacto */}
      ...
      {isMenuOpen && (
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gradient-to-b from-gray-50 to-white border-t border-gray-200">
            {navigation.map((item) => (
              item.href.startsWith('#') ? (
                <a
                  key={item.name}
